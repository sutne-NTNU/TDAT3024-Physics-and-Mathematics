function animate_AngularVelocity(X_all, I, L, time_interval, filename)
...
figure('visible','on')
title('Angular Velocity around each axis')
xlim([time_interval(1) time_interval(2)]);
ylim([-1.2 1.2]);
hold on

wb_x = animatedline('Color','blue','linewidth',2);
wb_y = animatedline('Color','red','linewidth',2);
wb_z = animatedline('Color','green','linewidth',2);

xlabel('Time (seconds)');
ylabel('Angular velocity around center of mass (rad/s)');
legend({'${\omega_{x}}$', '${\omega_{y}}$', '${\omega_{z}}$'}, 'Interpreter','latex', 'FontSize',18)
...
for i=0: frameGap : n-1
	...
	wb = angular_velocity(X_all{index}, I, L);
	
	t = h * index;
	addpoints(wb_x , t , wb(1));
	addpoints(wb_y , t , wb(2));
	addpoints(wb_z , t , wb(3));
	drawnow limitrate
